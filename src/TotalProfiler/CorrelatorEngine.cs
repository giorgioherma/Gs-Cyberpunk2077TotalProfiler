...
internal static class CorrelatorEngine
{
    ...
    public static Result Run(string rawRoot, string outDir)
    {
        ...
        File.WriteAllText(reportPath, BuildHtml(sync, health, perf, g.ByMod, c.ByMod, hitches, timeline), new UTF8Encoding(false));
    ...
    private static string BuildHtml(Dictionary<string,object?>sync,Dictionary<string,object?>health,Dictionary<string,object?>perf,List<ModRow>rs,List<Dictionary<string,object?>>cet,List<Dictionary<string,object?>>h,List<Dictionary<string,object?>>timeline)
    {
        var chart = timeline.Select(r => new
        {
            t = D(r,"bucket_start_ms") / 1000.0,
            ft = D(r,"capx_max_frametime_ms"),
            rs = D(r,"grsp_observed_exclusive_ms"),
            cet = D(r,"cet_observed_exclusive_ms")
        }).ToList();
        var chartJson = JsonSerializer.Serialize(chart);

        var sb=new StringBuilder("<!doctype html><html><head><meta charset='utf-8'><title>G's TOTAL Profiler Report</title><style>body{font-family:Segoe UI,Arial;background:#11141a;color:#e7ebf3;margin:28px}h1,h2{color:#fff}.cards{display:flex;gap:12px;flex-wrap:wrap}.card{background:#1b2029;border:1px solid #303846;border-radius:8px;padding:14px;min-width:170px}table{border-collapse:collapse;width:100%;background:#171b22;margin:12px 0 28px}th,td{border-bottom:1px solid #2d3440;padding:7px 9px;text-align:left;font-size:13px}th{background:#222833}.muted{color:#aeb7c4}.good{color:#81d39b}.warn{color:#f3c878}code{background:#202630;padding:2px 5px;border-radius:4px}.chartbox{background:#171b22;border:1px solid #303846;border-radius:8px;padding:12px;margin:12px 0 28px}.toolbar{display:flex;gap:8px;margin-bottom:8px}.toolbar button{background:#222833;color:#e7ebf3;border:1px solid #3b4657;border-radius:5px;padding:6px 10px;cursor:pointer}.legend{display:flex;gap:18px;flex-wrap:wrap;color:#aeb7c4;font-size:12px;margin-top:8px}.sw{display:inline-block;width:12px;height:3px;vertical-align:middle;margin-right:5px}.ft{background:#5ea2ff}.rs{background:#81d39b}.cet{background:#f3c878}canvas{width:100%;height:380px;display:block;background:#11141a;border-radius:5px}</style></head><body>");
        sb.AppendLine("<h1>G's Cyberpunk 2077 TOTAL Profiler</h1><p class='muted'>Native .NET combined profiling report. Evidence classes are synchronized signals, not causal verdicts.</p>");
        sb.AppendLine($"<div class='cards'><div class='card'><b>Sync</b><br><span class='{(Convert.ToString(sync["quality"])=="GOOD"?"good":"warn")}'>{H(sync["quality"])}</span><br>{F(sync["frametime_correlation"],6)} correlation</div><div class='card'><b>Average</b><br>{F(perf["average_fps"],2)} FPS<br>{F(perf["average_frametime_ms"],3)} ms</div><div class='card'><b>P95 / P99</b><br>{F(perf["p95_frametime_ms"],3)} / {F(perf["p99_frametime_ms"],3)} ms</div><div class='card'><b>Worst frame</b><br>{F(perf["maximum_frametime_ms"],3)} ms</div><div class='card'><b>≥33.3 / 50 / 100</b><br>{perf["frames_ge_33_3_ms"]} / {perf["frames_ge_50_ms"]} / {perf["frames_ge_100_ms"]}</div></div>");

        sb.AppendLine("<h2>Combined timeline</h2><div class='chartbox'><div class='toolbar'><button onclick='zoomAll()'>Full capture</button><button onclick='zoomWorst()'>Around largest hitch</button></div><canvas id='timelineChart'></canvas><div class='legend'><span><i class='sw ft'></i>CapFrameX max frametime</span><span><i class='sw rs'></i>GRSP observed REDscript exclusive</span><span><i class='sw cet'></i>CET observed exclusive</span></div></div><p class='muted'>The three lines are synchronized evidence layers. Do not add or subtract them as one CPU budget.</p>");

        sb.AppendLine("<h2>Synchronization</h2><table><tr><th>Metric</th><th>Value</th></tr>");
        foreach(var kv in sync)sb.AppendLine($"<tr><td>{H(kv.Key)}</td><td>{H(kv.Value)}</td></tr>");
        sb.AppendLine("</table><h2>Top REDscript owners</h2><table><tr><th>#</th><th>Owner</th><th>ms/s</th><th>Share</th><th>Active</th><th>Max frame</th><th>Pattern</th></tr>");
        foreach(var m in rs.Take(25))sb.AppendLine($"<tr><td>{m.Rank}</td><td>{H(m.Owner)}</td><td>{m.MsPerSec:F3}</td><td>{m.SharePct:F2}%</td><td>{m.ActivePct:F1}%</td><td>{m.MaxFrame:F3}</td><td>{H(m.Workload)}</td></tr>");
        sb.AppendLine("</table><h2>Top CET mods</h2><table><tr><th>Mod</th><th>ms/s</th><th>Share</th><th>One core</th><th>Max exclusive</th></tr>");
        foreach(var m in cet.Take(25))sb.AppendLine($"<tr><td>{H(m["mod"])}</td><td>{F(m["exclusive_ms_per_sec"],3)}</td><td>{F(m["share_pct"],2)}%</td><td>{F(m["one_core_pct"],2)}%</td><td>{F(m["max_exclusive_ms"],3)}</td></tr>");
        sb.AppendLine("</table><h2>Largest hitches</h2><table><tr><th>Frame</th><th>Frametime</th><th>REDscript</th><th>CET exact</th><th>Evidence</th><th>CET target</th></tr>");
        foreach(var r in h.Take(100))sb.AppendLine($"<tr><td>{r["capx_frame_index"]}</td><td>{F(r["capx_frametime_ms"],3)}</td><td>{F(r["grsp_exclusive_ms"],3)}</td><td>{F(r["cet_exact_exclusive_ms"],3)}</td><td>{H(r["evidence_class"])}</td><td>{H(S(r,"cet_exact_mod")+" / "+S(r,"cet_exact_target"))}</td></tr>");
        sb.AppendLine("</table><p class='muted'>Do not subtract GRSP and CET from frametime to infer a native remainder. Measurement domains can overlap.</p>");

        sb.Append("<script>const SERIES=").Append(chartJson).AppendLine(";");
        sb.AppendLine("const canvas=document.getElementById('timelineChart'),ctx=canvas.getContext('2d');let view0=SERIES.length?SERIES[0].t:0,view1=SERIES.length?SERIES[SERIES.length-1].t:1;function zoomAll(){view0=SERIES.length?SERIES[0].t:0;view1=SERIES.length?SERIES[SERIES.length-1].t:1;draw();}function zoomWorst(){if(!SERIES.length)return;let b=SERIES.reduce((a,x)=>x.ft>a.ft?x:a,SERIES[0]);view0=Math.max(0,b.t-3);view1=b.t+3;draw();}function draw(){const dpr=window.devicePixelRatio||1,W=Math.max(320,canvas.clientWidth),H=380;canvas.width=Math.floor(W*dpr);canvas.height=Math.floor(H*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,W,H);const pL=55,pR=12,pT=12,pB=30,vis=SERIES.filter(p=>p.t>=view0&&p.t<=view1);let ymax=16.67;for(const p of vis)ymax=Math.max(ymax,p.ft,p.rs,p.cet);ymax*=1.08;const span=Math.max(.001,view1-view0),x=t=>pL+(t-view0)/span*(W-pL-pR),y=v=>pT+(1-Math.min(v,ymax)/ymax)*(H-pT-pB);ctx.strokeStyle='#28323b';ctx.fillStyle='#8c9aa6';ctx.font='11px Segoe UI';ctx.lineWidth=1;for(let k=0;k<=5;k++){const v=ymax*k/5,yy=y(v);ctx.beginPath();ctx.moveTo(pL,yy);ctx.lineTo(W-pR,yy);ctx.stroke();ctx.fillText(v.toFixed(v>=100?0:1)+' ms',4,yy+4);}function line(key,color){ctx.strokeStyle=color;ctx.lineWidth=1.5;ctx.beginPath();let started=false;for(const p of vis){const xx=x(p.t),yy=y(p[key]);if(!started){ctx.moveTo(xx,yy);started=true;}else ctx.lineTo(xx,yy);}ctx.stroke();}line('ft','#5ea2ff');line('rs','#81d39b');line('cet','#f3c878');ctx.fillStyle='#8c9aa6';ctx.fillText(view0.toFixed(1)+' s',pL,H-8);const rt=view1.toFixed(1)+' s';ctx.fillText(rt,W-pR-ctx.measureText(rt).width,H-8);}window.addEventListener('resize',draw);draw();</script></body></html>");
        return sb.ToString();
    }
