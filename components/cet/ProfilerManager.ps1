...
$group.Size = New-Object System.Drawing.Size(760, 215)
$form.Controls.Add($group)

$lblStatus = New-Object System.Windows.Forms.Label
$lblStatus.Location = New-Object System.Drawing.Point(18, 28)
$lblStatus.Size = New-Object System.Drawing.Size(720, 128)
...
$btnRefresh.Location = New-Object System.Drawing.Point(625, 164)
...
$compatGroup.Location = New-Object System.Drawing.Point(20, 307)
...
$lblCompat.Text = "Use this if Scheduler integration fails or 0-Engine cannot be safely patched. 0-Engine stays installed and loaded exactly as-is; the Manager skips all 0-Engine/Scheduler changes. Core profiling still works."
...
$btnInstall.Location = New-Object System.Drawing.Point(20, 394)
...
$btnCollect.Location = New-Object System.Drawing.Point(265, 394)
...
$btnRestore.Location = New-Object System.Drawing.Point(555, 394)
$btnRestore.Size = New-Object System.Drawing.Size(225, 44)
$form.Controls.Add($btnRestore)

$btnOpen = New-Object System.Windows.Forms.Button
$btnOpen.Text = "Open Results Folder"
$btnOpen.Location = New-Object System.Drawing.Point(20, 453)
$btnOpen.Size = New-Object System.Drawing.Size(230, 36)
$form.Controls.Add($btnOpen)

$lblInfo = New-Object System.Windows.Forms.Label
$lblInfo.Location = New-Object System.Drawing.Point(20, 505)
$lblInfo.Size = New-Object System.Drawing.Size(760, 55)
$lblInfo.Text = "0-Engine is never assumed compatible merely because it exists. Existing PASS/Scheduler integration is preserved; recognizable unintegrated versions receive a temporary adaptive bridge built on their own init.lua. Core profiler mode leaves 0-Engine completely untouched."
...
function Refresh-Status {
    ...
    if (!(Test-GameRoot $root)) {
        $lblStatus.Text = "Game:       NOT FOUND`r`nCET:        -`r`n0-Engine:   -`r`nInit:       -`r`nScheduler:  -`r`nControls:   -`r`nLive CSVs:  -"
        $btnInstall.Enabled = $false; $btnCollect.Enabled = $false; $btnRestore.Enabled = $false
    ...
    $officialHash = ([string]$Manifest.targetCET.officialSha256).ToLowerInvariant()
    $profilerHash = ([string]$Manifest.targetCET.profilerSha256).ToLowerInvariant()
    $liveHash = Get-Sha256 $p.LiveAsi
    $cetText = if ($liveHash -eq $officialHash) { "OFFICIAL $($Manifest.targetCET.version)" } elseif ($liveHash -eq $profilerHash) { "PROFILER ACTIVE" } else { "UNKNOWN" }

    $managedState = Get-State $p
    $managed = if ($null -ne $managedState) { "YES" } else { "NO" }
    $zeroPresent = Test-Path -LiteralPath $p.ZeroInit -PathType Leaf
    $coreOnlySelected = ($chkCoreOnly.Checked -and $zeroPresent -and $null -eq $managedState)

    if ($null -ne $managedState -and [string]$managedState.zeroEngine.mode -eq "bypassed") {
        # Legacy alpha6 recovery path only.
        $zeroText = "BYPASSED - LEGACY MANAGED STATE"
        $initText = "0-ENGINE TEMPORARILY DISABLED"
        $schedText = "DISABLED"
    }
    elseif ($null -ne $managedState -and [string]$managedState.zeroEngine.mode -eq "core-only") {
        $zeroText = "FOUND - LEFT UNTOUCHED"
        $initText = "CORE PROFILER MODE"
        $schedText = "NOT MANAGED"
    }
    elseif ($coreOnlySelected) {
        $zeroText = "FOUND - WILL REMAIN UNTOUCHED"
        $initText = "CORE PROFILER MODE SELECTED"
        $schedText = "NOT MANAGED"
    }
    elseif ($zeroPresent) {
        $zeroText = "FOUND - UNVERIFIED"
        $initState = Get-ZeroInitState $p
        $initText = $initState.Text
        if ($initState.Kind -eq "integrated") {
            $schedText = (Get-ZeroSchedulerState $p).Text
        }
        elseif ($initState.Kind -eq "adaptive" -or $initState.Kind -eq "profiler-bridge") {
            $schedText = (Get-ZeroAdaptiveSchedulerState $p).Text
        }
        else {
            $schedText = "CHECK CORE PROFILER MODE"
        }
    }
    else {
        $zeroText = "NOT FOUND - CORE PROFILER ONLY"
        $initText = "-"
        $schedText = "-"
    }

    $controlsText = if (Test-Path -LiteralPath $p.Controls) { "PRESENT" } else { "NOT INSTALLED" }
    $resultCount = @(Get-LiveResults $p).Count

    $lblStatus.Text = "Game:       FOUND`r`nCET:        $cetText`r`n0-Engine:   $zeroText`r`nInit:       $initText`r`nScheduler:  $schedText`r`nControls:   $controlsText`r`nLive CSVs:  $resultCount    Managed install: $managed"

    $cetAllowed = ($liveHash -eq $officialHash -or $liveHash -eq $profilerHash)
    $structureAllowed = $true
    if ($zeroPresent -and !$coreOnlySelected -and $null -eq $managedState) {
        $kind = (Get-ZeroInitState $p).Kind
        $structureAllowed = ($kind -ne "unsafe")
    }
    $btnInstall.Enabled = ($cetAllowed -and $null -eq $managedState -and $resultCount -eq 0 -and $structureAllowed)
    $btnCollect.Enabled = ($resultCount -gt 0)
    $btnRestore.Enabled = ($null -ne $managedState)
    $chkCoreOnly.Enabled = ($zeroPresent -and $null -eq $managedState)
}

$btnBrowse.Add_Click({
    $d = New-Object System.Windows.Forms.FolderBrowserDialog
    $d.Description = "Select the Cyberpunk 2077 game folder"
    if ($d.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { $txtPath.Text = $d.SelectedPath; Refresh-Status }
})
...
$btnInstall.Add_Click({
    try {
        $p = Get-Paths $txtPath.Text.Trim()
        $coreOnly = [bool]$chkCoreOnly.Checked
        Install-Profiler $p $coreOnly
        Refresh-Status
        $s = Get-State $p
        if ([string]$s.zeroEngine.mode -eq "core-only") {
            $zeroMsg = "Core profiler mode is active. 0-Engine and its Scheduler were left completely untouched; no integration was injected or replaced."
        }
        elseif ([string]$s.zeroEngine.mode -eq "bypassed") {
            $zeroMsg = "Legacy alpha6 bypass state is active. Restore before making a new install."
        }
        elseif ([string]$s.zeroEngine.mode -eq "integrated") {
            $zeroMsg = "Existing 0-Engine Scheduler integration was preserved. Scheduler.lua mode: $($s.zeroEngine.scheduler.mode)."
        }
        elseif ([string]$s.zeroEngine.mode -eq "adaptive") {
            $zeroMsg = "Adaptive Scheduler integration was added to the user's existing 0-Engine transactionally. init.lua mode: $($s.zeroEngine.init.mode); CETProfilerScheduler.lua mode: $($s.zeroEngine.adaptiveScheduler.mode)."
        }
        else {
            $zeroMsg = "0-Engine was not found, so core CET profiling is active without Scheduler attribution."
        }
        Show-Info ("Profiler installed.`r`n" + $zeroMsg + "`r`nCETProfilerControls is temporary and will be removed on restore.")
...
$btnCollect.Add_Click({
    try {
        Assert-GameClosed
        $p = Get-Paths $txtPath.Text.Trim(); $dest = Collect-ResultsInternal $p $false; Refresh-Status
        Show-Info ("Results archived successfully.`r`n`r`nArchive folder:`r`n" + $dest + "`r`n`r`nLive CET profiler CSVs were cleared.")
    } catch { Show-Error $_.Exception.Message; Refresh-Status }
})
...
$btnRestore.Add_Click({
    try {
        $p = Get-Paths $txtPath.Text.Trim()
        $before = Get-State $p
        $wasCoreOnly = ($null -ne $before -and [string]$before.zeroEngine.mode -eq "core-only")
        $archived = Restore-Profiler $p; Refresh-Status
        $extra = if ($archived) { "`r`nFinal live results were archived to:`r`n$archived" } else { "" }
        $base = if ($wasCoreOnly) {
            "Original managed CET state restored. 0-Engine was left untouched throughout. CETProfilerControls was removed."
        } else {
            "Original managed CET/0-Engine state restored. CETProfilerControls was removed."
        }
        Show-Info ($base + $extra)
    } catch { Show-Error $_.Exception.Message; Refresh-Status }
})
